...
class PaymentController extends BaseController {
    ...
    public async payment(invoice : Invoice, card) {
        ...
        if(response.errorCode === "00") {
            const paymentTransaction : PaymentTransaction = new PaymentTransaction(response.transaction.command, response.transaction.transactionContent, response.transaction.createdAt, invoice.getId());
        }
        return response;
    }
